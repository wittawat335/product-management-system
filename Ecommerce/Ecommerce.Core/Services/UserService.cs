    ...
    public class UserService : IUserService
    {
        ...
        public async Task<Response<List<UserDTO>>> GetList(UserDTO request = null)
        {
            ...
            query = query
                ...
                .Where(x => x.Position.Status == Constants.Status.Active);
            if (request != null)
            {
                query = (request.UserId != null) 
                    ? query.Where(x => x.UserId == new Guid(request.UserId)) : query;
                query = (request.PositionId != null) 
                    ? query.Where(x => x.PositionId == request.PositionId) : query;
